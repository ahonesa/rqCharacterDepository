export default ({str, str_max, str_org, con, con_org, con_max, siz, siz_org, siz_max, int, int_org, int_max, pow, pow_org, pow_max, dex, dex_org, dex_max, cha, cha_org, cha_max}) => {
    ...
    const spiritCombatDamage = calculatespiritCombatDamage(characteristics.POW.current, characteristics.CHA.current)
    const dexStrikeRank = calculateDexStrikeRank(characteristics.DEX.current)
    const sizStrikeRank = calculateSizStrikeRank(characteristics.SIZ.current)
...
const calculatespiritCombatDamage = (pow, cha) => {
    const total = pow + cha

    if (total < 13) return "1D3"
    else if (total < 25) return "1D6"
    else if (total < 33) return "1D6+1"
    else if (total < 41) return "1D6+3"
    else if (total < 57) return "2D6+3"
    else {
        const diceNumber = 2 + Math.ceil((total - 56) / 16)
        return "" + diceNumber + "D6+" + (diceNumber + 1)
    }
}
...
const calculateDexStrikeRank = (dex) => {
    if (dex < 6) return 5
    else if (dex < 9) return 4
    else if (dex < 13) return 3
    else if (dex < 16) return 2
    else if (dex < 19) return 1
    else return 0
}

const calculateSizStrikeRank = (siz) => {
    if (siz < 7) return 3
    else if (siz < 15) return 2
    else if (siz < 22) return 1
    else return 0
}
